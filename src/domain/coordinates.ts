...
function sameMapPoint(first: MapPoint, second: MapPoint) {
  return Math.abs(first.x - second.x) < POINT_EPSILON && Math.abs(first.z - second.z) < POINT_EPSILON
}
...
export function normalizeBoundary(points: MapPoint[]) {
  ...
  if (
    normalized.length > 1 &&
    sameMapPoint(normalized[0], normalized[normalized.length - 1])
  ) {
    normalized.pop()
  }
...
export function getBusinessBoundaryMapPoints(business: Business) {
  return normalizeBoundary(
    business.boundary.map((coordinate) =>
      localPointFromGps(business.mapOrigin, coordinate, business.mapScaleMeters),
    ),
  )
}
